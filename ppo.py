...
class PPOAgent(nn.Module):
    ...
    def update(self, obs, actions, rewards, next_obs, terminals, action_logprob, value):
        ...
        actions = torch.squeeze(torch.from_numpy(np.array(actions))).to(self.device)
        returns = torch.from_numpy(self.discount_rewards(np.array(rewards), np.array(terminals))).to(self.device, dtype=torch.float32)
        next_obs = torch.from_numpy(np.array(next_obs)).to(self.device)
        terminals = torch.from_numpy(np.array(terminals))
        old_action_logprob = torch.squeeze(torch.from_numpy(np.array(action_logprob))).to(self.device, dtype=torch.float32)
        old_state_value = torch.squeeze(torch.from_numpy(np.array(value))).to(self.device, dtype=torch.float32)
        ...
        for i in range(self.policy_update_steps):
            ...
            for j in range(number_of_strides):
                ...
                actor_loss2 = -sample_advantages * torch.clamp(ratio, 1.0-self.eps_clip, 1.0+self.eps_clip) #calulate the actor loss with clipping the sample_advantages * ratio to policy from diverging too much
                pi_loss = torch.max(actor_loss1, actor_loss2).mean() - self.entropy*sample_entropy.mean() #calculates the actor loss based on a clipped policy and trajectory advantages
